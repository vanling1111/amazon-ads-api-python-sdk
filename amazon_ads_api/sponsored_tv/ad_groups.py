...
class SponsoredTVAdGroupsAPI(BaseAdsClient):
    ...
    async def create_ad_groups(
        ...
        """创建广告组"""
        result = await self.post("/st/adGroups", json_data={"adGroups": ad_groups})
    ...
    async def list_ad_groups(
        ...
        *,
        ...
        result = await self.post("/st/adGroups/list", json_data=request_body)
    ...
    async def update_ad_groups(
        ...
        """更新广告组"""
        result = await self.put("/st/adGroups", json_data={"adGroups": ad_groups})
        return result if isinstance(result, dict) else {}
    ...
    async def delete_ad_groups(
        ...
        """删除广告组"""
        result = await self.post("/st/adGroups/delete", json_data={"adGroupIds": ad_group_ids})
        return result if isinstance(result, dict) else {}
